...
describe('PaginationComponent', () => {
  ...
  it('should emit array of numbers when different pages clicked', () => {
    const pageContainers = fixture.debugElement.queryAll(
      By.css('[data-testid="page-container"]')
    );
    let clickedPage: number[] = [];

    component.pageChangeEvent.pipe(take(3), toArray()).subscribe((page) => {
      clickedPage = page;
    });
    pageContainers[0].triggerEventHandler('click');
    pageContainers[1].triggerEventHandler('click');
    pageContainers[2].triggerEventHandler('click');
    expect(clickedPage).toEqual([1, 2, 3]);
  });
});
